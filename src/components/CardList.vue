...
export default {
    ...
    methods: {
        getArrayLength() {
            let number = this.listaCards.length
            return number
        }
    }
}
...
<template>
    <div class="contenitore">
        <div class="founded-card">
            <h3>Found {{ getArrayLength }} Card</h3>
        </div>
        <div class="card-container">

            <!-- <SingleCard v-for="card in listaCards" :img="card.card_images.image_url" :description="card.name"
                :model="card.archetype" /> -->
            <SingleCard v-for="card in store.cards" :img="card.card_images.image_url" :description="card.name"
                :model="card.archetype" />
